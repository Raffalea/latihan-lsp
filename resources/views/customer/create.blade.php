...
<div class="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-12 bg-dark-800 rounded-[3rem] overflow-hidden shadow-2xl border border-white/5">
    ...
    <div class="lg:col-span-7 p-12">
        ...
        <form action="{{ route('reservasi.store') }}" method="POST" class="space-y-6">
            ...
            <button type="submit"
                    class="w-full bg-white hover:bg-gold-500 text-black font-black py-4 rounded-xl transition-all uppercase tracking-widest text-[11px] mt-4">
                Confirm Booking
            </button>
        </form>
    </div>
...
<script>
    ...
    function updatePrice() {
        if (checkIn.value && checkOut.value) {
            ...
            if (diff > 0) {
                document.getElementById('night-count').innerText = diff + ' Nights';
                document.getElementById('total-preview').innerText =
                    'Rp ' + (diff * price).toLocaleString('id-ID');
            }
        }
    }
